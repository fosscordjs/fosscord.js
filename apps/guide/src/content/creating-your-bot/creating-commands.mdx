...
- _`commands`_: An array of commands to register. The [slash command builder](/popular-topics/builders.md#slash-command-builders) from _`spacebar.js`_ is used to build the data for your commands
...
const { REST, SlashCommandBuilder, Routes } = require('spacebar.js');
...
Note that servers are referred to as "guilds" in the Discord API and spacebar.js library. _`interaction.guild`_ refers to the guild the interaction was sent in (a <DocsLink path="class/Guild" /> instance), which exposes properties such as _`.name`_ or _`.memberCount`_.
...
<DiscordMessages rounded>
	...
	>
		<p>Server name: spacebar.js Guide</p>
		<p>Total members: 2</p>
	</DiscordMessage>
</DiscordMessages>
